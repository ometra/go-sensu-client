...
func NewPluginProcessor(w io.Writer, statStore string) *PluginProcessor {
	...
	proc.stopCollectingOnNoConnection = false
...
func (p *PluginProcessor) Init(q MessageQueuer, config *Config) error {
	...
	p.ack = make(chan uint64, 1);
	p.nack = make(chan uint64, 1);

	// make this a reliable connection
	err = q.NotifyConfirm(p.ack, p.nack)

	if nil != err {
        p.logger.Println("Reliable publishing to AMQP not available... ")
        p.reliable = false
    } else {
        p.reliable = true
	}
...
// loads the results from a disk store and puts them in the result queue
		...
		case result := <-p.results:
			if result.HasOutput() {
                ...
                if p.reliable {
                    ...
                    select {
                    case <-p.ack:
                        p.logger.Println("Results Sent Successfully!",)
                    case tag = <-p.nack:
                        p.logger.Printf("Results were not sent successfully, requeuing, %d", tag)
                        p.results <- result
                    }
                }
			}
